...
contract Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(newOwner != address(0), "make sure newOwner is not a contract");
        // DONE: add functionality to transfer control of the contract to a newOwner.
        _owner = newOwner;
        emit NewOwnership(msg.sender, newOwner);
    }
    //  5) DONE: create an event that emits anytime ownerShip is transfered (including in the constructor)
    event NewOwnership(address indexed oldOwner, address indexed newOwner);
}
...
contract Pausable is Ownable
{
    ...
    {
        //DONE: check if the contract is paused
        require(status != _paused, "new paused state must be different than current state");
        _paused = status;
        if(_paused) emit Pause(msg.sender); else emit Unpause(msg.sender);
    }
    ...
    //  5) DONE: create a Paused & Unpaused event that emits the address that triggered the event
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(ownerOf(tokenId) != to, "cannot approve to the owner");
        // DONE: require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == _owner || isApprovedForAll(_owner, msg.sender), "only the owner or operator can approve");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(!_exists(tokenId) && to != address(0), "tokenId already exists or to address is invalid");
...
//  1) DONE:Pass in appropriate values for the inherited ERC721Metadata contract
//      - make the base token uri: https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/
contract CoolHousesToken is ERC721Metadata("CustomERC721Token", "CHT", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") 
{
    // DONE: 2) create a public mint() that does the following:
